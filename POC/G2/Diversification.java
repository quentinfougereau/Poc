public class Diversification {
	...
	public static byte[] calcule_la_clef_etendue(byte[] key) {
		...
		byte extended_key[] = new byte[longueur_de_la_clef_etendue];

		/* key est recopiée dans extended_key */
		...
		for (int i = Nk * 4; i < longueur_de_la_clef_etendue; i += 4) {
			...
			while (j < 4) {
				tmp2[j] = extended_key[(i + j) - (4 * Nk)];
				j++;
			}
			tmp = xor(tmp2, tmp);
			/* Copie de tmp dans W */
			for (int l = 0; l < 4; l++) {
				extended_key[i + l] = tmp[l];
			}
